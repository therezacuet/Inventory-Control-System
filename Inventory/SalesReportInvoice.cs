...
namespace Inventory
{
    public partial class SalesReportInvoice : Form
    {
        ...
        public SalesReportInvoice()
        {
            InitializeComponent();
        }

        private void SalesReportInvoice_Load(object sender, EventArgs e)
        {
            //for (int i = 60; i > 0; i--)
            //{
            ...
            for (int i = 0; i < reportGridView.Rows.Count; i++)
            {
                ...
                var pt = reportGridView.Rows[i].Cells[5].Value.ToString();
                float.TryParse(pt, out t);
                sum += t;
            }
            totalAmount = sum.ToString();

            totalPurLabel.Text = "Total Sales: " + sum.ToString() + " TK";
                ...
                while (iRow <= reportGridView.Rows.Count - 1)
                {
                    ...
                    else
                    {
                        if (bNewPage)
                        {
                            ...
                            e.Graphics.DrawString("Sales Report", new Font(reportLabel.Font, FontStyle.Bold),
                                    Brushes.Black, e.MarginBounds.Left + 225, e.MarginBounds.Top -
                                    e.Graphics.MeasureString("", new Font(reportGridView.Font,
                                    FontStyle.Bold), e.MarginBounds.Width).Height + 100);

                            e.Graphics.DrawString("Total Sales : " + totalAmount + " TK", new Font(totalPurLabel.Font, FontStyle.Bold),
                                    Brushes.Black, e.MarginBounds.Left + 230, e.MarginBounds.Top -
                                    e.Graphics.MeasureString("", new Font(reportGridView.Font,
