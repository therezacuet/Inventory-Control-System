...
namespace Inventory
{
    public partial class PurchaseReportInvoiceForm : Form
    {
        ...
        public PurchaseReportInvoiceForm()
        {
            InitializeComponent();
        }

        private void PurchaseReportInvoiceForm_Load(object sender, EventArgs e)
        {
            //for (int i = 60; i > 0; i--)
            //{
            ...
            totalPurLabel.Text = "Total Purchase: "+sum.ToString()+" TK";
                ...
                while (iRow <= reportGridView.Rows.Count - 1)
                {
                    ...
                    else
                    {
                        if (bNewPage)
                        {
                            ...
                            e.Graphics.DrawString("Purchase Report", new Font(reportLabel.Font, FontStyle.Bold),
                                    Brushes.Black, e.MarginBounds.Left + 225, e.MarginBounds.Top -
                                    e.Graphics.MeasureString("", new Font(reportGridView.Font,
                                    FontStyle.Bold), e.MarginBounds.Width).Height + 100);

                            e.Graphics.DrawString("Total Purchase : " + totalAmount+" TK", new Font(totalPurLabel.Font, FontStyle.Bold),
                                    Brushes.Black, e.MarginBounds.Left + 240, e.MarginBounds.Top -
                                    e.Graphics.MeasureString("", new Font(reportGridView.Font,
